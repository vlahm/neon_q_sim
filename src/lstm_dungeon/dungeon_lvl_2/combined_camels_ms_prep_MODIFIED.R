...
run_prediction_routine_across_sites = if(strategy %in% c('specialist')) T else F
...
sites_for_routine <- grep('WLOU|BIGC|BLDE|PRIN|CARI|ARIK|OKSR|CUPE|TOMB|FLNT|SYCA|HOPB|TECR|BLUE|REDB|KING|WALK|BLWA|MCDI|MCRA|MAYF|LECO|COMO|MART|LEWI|POSE|GUIL', paste0(neon_sites, '_MANUALQ'), value = T) #all specialists
...
for(ix in siterange){
    ...
    # specialist_finetune_site_elect = sub('_MANUALQ', '', routine_site) #----------------------------!!!!!!!!!!! SPECIALIST? and def NHM
...
addtl_config_list <- list(
    ...
    run_note = 'DCC generalist search 30x',
    ...
    # pretrained_model = '/home/mike/git/macrosheds/qa_experimentation/imputation/src/nh_methods/runs/runs_1628-1657_pretrain_2309_212115/', #nhm with pet
        ...
        if(length(addtl_config_list$finetune_basins) > 1 || ! is.na(addtl_config_list$finetune_basins)){
            ...
            if(run_prediction_routine_across_sites){
                if((! is.na(addtl_config_list$nhm_lineup) && addtl_config_list$nhm_lineup == 2) ||
                   addtl_config_list$configure_specialist){
                    ...
                    finetune_basin_file <- sub('continue', 'test', finetune_basin_file) #HMM?
                }
            }
            ...
            if(run_prediction_routine_across_sites){
                cfg2$per_basin_test_periods_file <- gsub('finetune_train_ranges',
                                                         'test_ranges',
                                                         cfg2$per_basin_train_periods_file) #HMM?
            } else {
                cfg2$per_basin_test_periods_file <- gsub('finetune_train_ranges',
            ...
            write_lines(addtl_config_list$finetune_basins, ifelse(run_prediction_routine_across_sites, #HMM?
...
# reticulate::use_python(python_executable_path)
# reticulate::source_python(pickle_script,
#                           envir = NULL,
#                           convert = FALSE)
